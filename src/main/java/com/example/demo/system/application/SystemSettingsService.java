...
public class SystemSettingsService implements RuntimeSettingsProvider {
     ...
     * 列出全部设置记录（面向数据导入/导出）。
     */
    public List<SystemSettings> listSettings() {
        return settingsMapper.selectList(null);
    }

    /**
     * 用给定列表替换全部设置；用于数据导入恢复。
     */
    @Transactional
    @Caching(evict = {
    ...
    public void replaceSettings(List<SystemSettings> settings) {
        settingsMapper.delete(null);
        if (settings == null) {
            return;
        }
        settings.forEach(item -> {
            item.setId(null);
            settingsMapper.insert(item);
        });
    }

    /**
     * 批量按 category+key upsert 设置项；已存在的更新值与描述，未存在的插入。
     */
    @Transactional
    @Caching(evict = {
            @CacheEvict(cacheNames = CacheConfig.SETTINGS_ALL, allEntries = true),
    ...
    public void upsertSettings(List<SystemSettings> settings) {
        if (settings == null) {
            return;
        }
        settings.forEach(item -> {
            SystemSettings existing = settingsMapper.selectByCategoryAndKey(item.getCategory(), item.getConfigKey());
            if (existing == null) {
                item.setId(null);
                settingsMapper.insert(item);
            } else {
