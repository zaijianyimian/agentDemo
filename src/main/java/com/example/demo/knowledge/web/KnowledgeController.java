package com.example.demo.knowledge.web;
...
import com.example.demo.knowledge.application.RagService;
...
public class KnowledgeController {
    ...
    /**
     * 获取知识库列表。30 秒 Caffeine 缓存，sync=true 让并发同 key 请求只查一次 DB。
     */
    @GetMapping("/list")
    @Cacheable(cacheNames = CacheConfig.KNOWLEDGE_LIST, sync = true)
     ...
     */
    @GetMapping("/{id}")
    @Cacheable(cacheNames = CacheConfig.KNOWLEDGE_DETAIL, key = "#id", sync = true)
     ...
     */
    @PostMapping
    @CacheEvict(cacheNames = CacheConfig.KNOWLEDGE_LIST, allEntries = true)
     ...
     */
    @PutMapping("/{id}")
    @CacheEvict(cacheNames = {CacheConfig.KNOWLEDGE_LIST, CacheConfig.KNOWLEDGE_DETAIL}, allEntries = true)
