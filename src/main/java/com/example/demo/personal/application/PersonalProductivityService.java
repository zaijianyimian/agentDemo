 ...
 * 个人生产力业务服务。
 *
 * <p>Java 仅汇总任务、日程、聊天历史与系统设置。Code Agent、AI 报告与自动总结已经迁移
...
public class PersonalProductivityService {
    ...
    public Map<String, Object> insights() {
        ...
        List<ScheduledTask> tasks = scheduledTaskService.listTasks().stream()
                .filter(item -> Long.valueOf(ownerId).equals(item.getUserId())).toList();
        List<ScheduleEvent> schedules = scheduleEventService.listAll().stream()
                .filter(item -> Long.valueOf(ownerId).equals(item.getUserId())).toList();
        List<ChatMessageEntity> messages = chatHistoryService.getAllMessages().stream()
                .filter(item -> Long.valueOf(ownerId).equals(item.getUserId())).toList();
