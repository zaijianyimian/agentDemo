 ...
 * 邮件监听服务。
 *
 * <p>负责实际建立 IMAP/POP3 连接、维护 Folder、轮询新邮件并发布 {@link EmailReceivedEvent}。
...
public class EmailListenerService {
    ...
    /**
     * 已处理邮件的去重集合。Caffeine 提供真正的 LRU + 上限，避免无限增长导致 OOM。
     * 7 天未访问即淘汰（与 typical 邮件拉取间隔一致）。
     */
    private final Cache<String, Boolean> processedMessageKeys = Caffeine.newBuilder()
            .maximumSize(50_000)
            .expireAfterAccess(7, TimeUnit.DAYS)
            .build();

    /**
     * 最近处理的邮件 messageId → EmailMessage。供前端 EmailDetail 页查询邮件头/正文；
     * Caffeine LRU + size 上限防止内存膨胀。
     */
    private final Cache<String, EmailMessage> recentMessageCache = Caffeine.newBuilder()
            .maximumSize(RECENT_MESSAGE_CACHE_SIZE)
            .expireAfterWrite(1, TimeUnit.HOURS)
            .build();
    private static final int RECENT_MESSAGE_CACHE_SIZE = 200;
    ...
    public EmailListenerService(
            ...
            EmailListenerConfigSupport emailListenerConfigSupport,
        ...
        this.emailListenerConfigSupport = emailListenerConfigSupport;
    ...
    public void startListener(EmailConfig config) {
        if (emailListenerManager != null) {
            emailListenerManager.start(config);
            return;
        }
    ...
    private void processNewMessage(Message message, EmailConfig config, String trigger) {
        try {
            ...
            if (processedMessageKeys.asMap().putIfAbsent(messageKey, Boolean.TRUE) != null) {
            ...
            publishEmailReceived(emailMessage, trigger);
    ...
    /**
     * 发布新邮件事件，由日程抽取、前端通知等下游模块各自监听。
     */
    private void publishEmailReceived(EmailMessage emailMessage, String trigger) {
        cacheRecentMessage(emailMessage);
        eventPublisher.publishEvent(new EmailReceivedEvent(emailMessage, trigger, LocalDateTime.now()));
    }

    /**
     * 写入 recentMessageCache。Caffeine 自动 LRU 淘汰，无需手工处理。
     */
    private void cacheRecentMessage(EmailMessage emailMessage) {
        String key = emailMessage.getMessageId();
        if (key == null || key.isBlank()) {
            return;
        }
        recentMessageCache.put(key, emailMessage);
    }

    /**
     * 前端 EmailDetail 页面按 messageId 查询邮件头/正文。
     * 仅查询最近 RECENT_MESSAGE_CACHE_SIZE 条；查不到返回 null。
     */
    public EmailMessage peekMessage(String messageId) {
        if (messageId == null) {
            return null;
        }
        return recentMessageCache.getIfPresent(messageId);
    }
     ...
     * 提取 text/plain、text/html 内容以及所有附件（落盘到附件目录）。
     */
    private EmailMessage parseMessage(Message message, EmailConfig config) throws MessagingException, IOException {
        ...
        parseContent(message, builder, attachments, messageId, config);
     ...
     * 递归解析邮件 Part 树：
     * <ul>
     *   <li>text/plain / text/html 收集到 builder</li>
     *   <li>disposition 为 attachment 或 inline 且带文件名 / Content-ID 的 BodyPart 落盘并加入附件列表</li>
     *   <li>Multipart 时逐 BodyPart 递归</li>
     * </ul>
     */
    private void parseContent(Part part, EmailMessage.EmailMessageBuilder builder,
        ...
        if (content instanceof String) {
            ...
            // 一些邮件 text/plain 部分 disposition=attachment 但实质是文本正文，保留正文
            return;
        }
        ...
        // 既不是 String 也不是 Multipart 的就是附件/内联资源（如 application/octet-stream）
        if (part instanceof BodyPart bodyPart) {
            handleAttachment(bodyPart, attachments, messageId, config);
        }
    }

    /**
     * 判定 BodyPart 是否是附件或内联资源，是则落盘并加入 attachments 列表。
     */
    private void handleAttachment(BodyPart bodyPart, List<EmailMessage.Attachment> attachments,
                                  String messageId, EmailConfig config) {
        String fileName = null;
        try {
            String disposition = bodyPart.getDisposition();
            fileName = bodyPart.getFileName();
            String contentId = bodyPart.getHeader("Content-ID") == null ? null : bodyPart.getHeader("Content-ID")[0];
            ...
            // disposition 缺失但有文件名或 Content-ID 时也按附件处理
            if (!isAttachment && !isInline && !hasFileName && contentId == null) {
                return;
            }
            // 纯正文 text/* 已经在 parseContent 前面处理过，这里只处理真正的附件/内联资源
            ...
            log.warn("[{}] 附件落盘失败: file={}, err={}",
                    config.getEmail(), fileName, e.getMessage());
        }
    }
    ...
    public Map<Long, Map<String, Object>> getListenerStatus() {
        Map<Long, Map<String, Object>> managedStatus = emailListenerManager.status();
        if (!managedStatus.isEmpty()) {
     ...
     * 用于配置保存前的连通性预检。
     *
     * <p>基础 SSRF 防护：禁止目标解析到私网/loopback/链路本地地址，
     * 避免攻击者用此接口探测内网主机。允许显式白名单 IP 段以便用户自托管。</p>
     */
    public NetworkCheckResult checkNetworkConnectivity(String host, Integer port, int timeoutMs) {
        ...
        try {
            ...
            if (isBlockedAddress(address)) {
                return new NetworkCheckResult(false,
                        "禁止访问私网/loopback 地址",
    ...
    /**
     * 基础 SSRF 防护：拒绝 loopback、site-local、link-local、multicast 等"内网/特殊用途"地址。
     * 阻止已登录用户通过此接口对内网主机做 TCP 端口探测。
     */
    private static boolean isBlockedAddress(InetAddress address) {
        ...
        // 额外拦截 IPv4 私有段（1.0.0.0/8、100.64.0.0/10 CGNAT 等）
        byte[] raw = address.getAddress();
        if (raw.length == 4) {
            int b0 = raw[0] & 0xFF;
            // 100.64.0.0/10 (CGNAT)
            if (b0 == 100 && (raw[1] & 0xFF) >= 64 && (raw[1] & 0xFF) <= 127) {
                return true;
            }
        }
        return false;
