...
import com.example.demo.auth.dto.PuzzleCaptchaResponse;
import com.example.demo.auth.dto.PuzzleCaptchaVerifyResponse;
...
@Service
/**
 * 拼图（滑块）图形验证码服务。生成背景图与目标拼图的 SVG、通过比对滑块位置校验，
 * 并签发一次性 ticket 给后续登录 / 注册接口消费。
 */
public class PuzzleCaptchaService {
    ...
    /**
     * 创建一组新的拼图验证码，返回背景图、拼图图及 captchaId。
     */
    ...
    /**
     * 在登录/注册等接口中消费 ticket，无效或缺失时抛出异常供前端提示重新验证。
     */
