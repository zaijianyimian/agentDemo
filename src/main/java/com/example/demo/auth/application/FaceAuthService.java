...
import com.example.demo.auth.persistence.UserFaceProfileMapper;
...
/**
 * 人脸认证服务。基于 LBP/灰度直方图等经典特征做人脸检测、特征提取与余弦相似度比对，
 * 不依赖第三方模型；特征模板以 JSON 形式持久化并保留至多 3 张高质量历史样本。
 */
public class FaceAuthService {
    ...
    @Value("${app.auth.face.threshold:0.55}")
    ...
    /**
     * 注册或重新注册用户人脸特征。会自动与历史模板按质量合并，保留高质量样本。
     */
    ...
    /**
     * 计算上传人脸与已存模板的最大余弦相似度（不直接判定通过/失败），供前端调试展示。
     */
    ...
    /**
     * 登录场景下的人脸校验。与阈值比较，相似度不足抛出非法参数异常。
     */
